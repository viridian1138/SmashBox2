...
class RightPunchDownJumpStrategy(Strategy.Strategy):
    ...
    def init(self, tmpObj : SceneGenerator.SceneGenerator , strtTimeI : float ) :
        ...
        if True :
            ...
            tobj.strtPos = Vec3.Vec3();
            tobj.endPos = Vec3.Vec3();
            tobj.strtPos.set(startXA, tmpObj.offenseHeight * (tmpObj.abdomenHeight / tmpObj.idealHumanHeadHeights ) + 2 * startXA, tmpObj.backDistBucket);
            tobj.endPos.set(startXA, tmpObj.offenseHeight * (tmpObj.abdomenHeight / tmpObj.idealHumanHeadHeights ) + 2 * startXA, 0);
            tobj.strtTime = strtTimeA;
            tobj.endTime = tobj.strtTime + tmpObj.flightTimeKick;
            self.enableStart(tmpObj, tobj);
            self.randRotateFar(tmpObj, randx, tobj.strtPos);
